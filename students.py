...
def get_all_student(current_user=Depends(get_current_user),db: Session = Depends(get_db)):
    message=get_all_student_from_db( current_user=current_user,db=db)
    return message

@student_router.post('/student')
def create_new_student(*,item:CreateNewStudent,current_user=Depends(get_current_user),db: Session = Depends(get_db)):
